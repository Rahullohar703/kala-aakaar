...
import dyp07 from '@/assets/dyp-07.jpg';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const FeaturedProjects = () => {
  ...
  const projects: Project[] = [
    ...
    },
    {
      ...
      description: 'Modern corporate interior featuring sculptural wood elements, flowing LED ceiling designs, and sophisticated meeting spaces.',
      gallery: [dyp01, dyp02, dyp03, dyp04, dyp05, dyp06, dyp07],
    },
  ];
      ...
      <div ref={ref} className="container-luxury">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 lg:gap-8 mb-10 md:mb-12">
          <div className="overflow-hidden">
            <h2 className={`font-cormorant text-2xl md:text-3xl lg:text-4xl text-champagne font-normal italic transition-all duration-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
              Featured Projects
            </h2>
          </div>
          
          <div className={`lg:max-w-md transition-all duration-500 delay-100 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
            <p className="font-cormorant text-base md:text-lg text-foreground leading-relaxed mb-2">
            ...
            <p className="font-cormorant text-base md:text-lg text-muted-foreground leading-relaxed">
          ...
          <button
            ...
            className={`btn-luxury self-start whitespace-nowrap transition-all duration-500 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
          >
        ...
        {/* Projects Grid */}
        <div className="grid md:grid-cols-2 gap-x-8 lg:gap-x-12 gap-y-10 md:gap-y-12">
          {projects.map((project, index) => (
            ...
            >
              ...
              <div className="relative aspect-[4/3] overflow-hidden rounded-md mb-4">
                {/* Overlay */}
                <div className={`absolute inset-0 bg-charcoal/15 z-10 transition-opacity duration-300 ${hoveredIndex === index ? 'opacity-0' : 'opacity-100'}`} />
                ...
                {/* Hover Reveal Text */}
                <div className={`absolute inset-0 z-20 flex items-center justify-center transition-opacity duration-300 ${hoveredIndex === index ? 'opacity-100' : 'opacity-0'}`}>
                  <span className="font-jost text-xs tracking-[0.15em] uppercase text-ivory bg-charcoal/50 px-4 py-2 backdrop-blur-sm">
                    View Project
                  </span>
                </div>

                {/* Corner Accents on Hover */}
                <div className={`absolute top-3 left-3 w-6 h-px bg-champagne transition-transform duration-300 origin-left ${hoveredIndex === index ? 'scale-x-100' : 'scale-x-0'}`} />
                <div className={`absolute top-3 left-3 w-px h-6 bg-champagne transition-transform duration-300 delay-75 origin-top ${hoveredIndex === index ? 'scale-y-100' : 'scale-y-0'}`} />
                <div className={`absolute bottom-3 right-3 w-6 h-px bg-champagne transition-transform duration-300 origin-right ${hoveredIndex === index ? 'scale-x-100' : 'scale-x-0'}`} />
                <div className={`absolute bottom-3 right-3 w-px h-6 bg-champagne transition-transform duration-300 delay-75 origin-bottom ${hoveredIndex === index ? 'scale-y-100' : 'scale-y-0'}`} />
              </div>

              {/* Content */}
              <p className={`text-xs tracking-[0.15em] uppercase text-charcoal mb-1.5 transition-transform duration-300 ${hoveredIndex === index ? 'translate-x-1' : 'translate-x-0'}`}>
                {project.location}
              </p>
              <h3 className={`font-cormorant text-xl md:text-2xl text-champagne font-normal italic transition-transform duration-300 ${hoveredIndex === index ? 'translate-x-1' : 'translate-x-0'}`}>
                {project.title}
              </h3>
              
              {/* Animated Underline */}
              <div className={`w-full h-px bg-champagne/30 mt-3 origin-left transition-all duration-500 ${hoveredIndex === index ? 'scale-x-100 bg-champagne/60' : 'scale-x-50'}`} />
            </div>
          ))}
        </div>
      </div>
          ...
          {selectedProject && (
            <div className="flex flex-col max-h-[90vh]">
              <DialogHeader className="p-3 sm:p-4 md:p-6 pb-2 sm:pb-3 md:pb-4 border-b border-champagne/10 flex-shrink-0">
                ...
                <DialogTitle className="font-cormorant text-lg sm:text-xl md:text-2xl lg:text-3xl text-champagne font-normal italic">
                  {selectedProject.title}
                </DialogTitle>
                <p className="font-cormorant text-xs sm:text-sm md:text-base text-foreground/80 mt-1 sm:mt-2">
